...
from third_part.mmdeploy.mmdeploy.backend.tensorrt import from_onnx
from third_part.mmdeploy.mmdeploy.backend.tensorrt.utils import get_trt_log_level
from third_part.mmdeploy.mmdeploy.utils import (get_common_config, get_model_inputs,
                            get_root_logger, load_config)


def parse_args():
    parser = argparse.ArgumentParser(description='Convert ONNX to TensorRT.')
    parser.add_argument('--deploy_cfg', default='')
    parser.add_argument('--onnx_path', default='')
    parser.add_argument('--output_prefix', default='output TensorRT engine prefix')
