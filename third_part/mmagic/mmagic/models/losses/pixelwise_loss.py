...
class TXYLOSS(nn.Module):
    ...
    #     self.loss_weight = loss_weight
    #     self.toY = toY
    #     self.coef = torch.tensor([65.481, 128.553, 24.966]).reshape(1, 3, 1, 1) / 255.0  # 假设这里已经规范化到[0, 1]
    #     self.scale = 10 / np.log(10)

    # def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    #     if self.toY:
    #         pred = self.rgb_to_y(pred)
    #         target = self.rgb_to_y(target)

    #     # 确保输入和目标的动态范围在 [0, 1] 之间
    #     pred = torch.clamp(pred, 0, 1)
    #     target = torch.clamp(target, 0, 1)

    #     mse = torch.mean((pred - target) ** 2)
    #     return self.loss_weight * mse

    # def rgb_to_y(self, x):
    #     # 将 RGB 图像转换为 YCbCr 颜色空间的 Y 通道
    #     return torch.sum(x * self.coef, dim=1, keepdim=True)
    def __init__(self, loss_weight: float = 1.0, toY: bool = False) -> None:
        super(TXYLOSS, self).__init__()
        self.loss_weight = loss_weight
        self.toY = toY
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # 规范化预测图像到 [0, 1] 范围，并确保值在范围内
        pred = torch.clamp(pred / 255.0, 0, 1)
    
        # 确保输入和目标的动态范围在 [0, 1] 之间
        assert torch.min(pred) >= 0 and torch.max(pred) <= 1
        assert torch.min(target) >= 0 and torch.max(target) <= 1
    
        mse = torch.mean((pred - target) ** 2, dim=(1, 2, 3))
        psnr = -self.scale * torch.log1p(mse + 1e-5)  # 增加一个更大的正数以避免负的 PSNR 值
